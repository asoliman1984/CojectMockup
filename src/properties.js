...
export function showProperties(id, elementsArray) {
    ...
    if (item) {
        ...
        elementX.onchange = () => updateElementPosition(item, 'x', elementX.value, elementsArray);
        elementY.onchange = () => updateElementPosition(item, 'y', elementY.value, elementsArray);
        elementWidth.onchange = () => updateElementSize(item, 'width', elementWidth.value, elementsArray);
        elementHeight.onchange = () => updateElementSize(item, 'height', elementHeight.value, elementsArray);
        ...
        if (item.type === 'button') {
            addCustomProperty(customPropertiesContainer, 'Button Text', item.element, 'text', item.element.querySelector('text'));
            addCustomProperty(customPropertiesContainer, 'Button Color', item.element, 'color', item.element.querySelector('rect'), 'fill');
            addCustomProperty(customPropertiesContainer, 'Text Color', item.element, 'color', item.element.querySelector('text'), 'fill');
        } else if (item.type === 'textbox') {
            addCustomProperty(customPropertiesContainer, 'Textbox Text', item.element, 'text', item.element.querySelector('text'));
        } else if (item.type === 'grid') {
            addCustomProperty(customPropertiesContainer, 'Number of Rows', item, 'number', item, 'rows', true);
            addCustomProperty(customPropertiesContainer, 'Number of Columns', item, 'number', item, 'cols', true);
            addCustomProperty(customPropertiesContainer, 'Has Header', item, 'checkbox', item, 'hasHeader', true);
        }
...
function addCustomProperty(container, label, element, type, target, attribute, isGridProperty = false) {
    ...
    if (type === 'text') {
        ...
        inputElement.oninput = () => target.setAttribute(attribute, inputElement.value);
    } else if (type === 'number') {
        inputElement = document.createElement('input');
        inputElement.type = 'number';
        inputElement.className = 'form-control';
        inputElement.value = target[attribute];
        inputElement.oninput = () => updateGridProperty(target, attribute, parseInt(inputElement.value, 10), element, isGridProperty);
    } else if (type === 'checkbox') {
        inputElement = document.createElement('input');
        inputElement.type = 'checkbox';
        inputElement.className = 'form-control';
        inputElement.checked = target[attribute];
        inputElement.onchange = () => updateGridProperty(target, attribute, inputElement.checked, element, isGridProperty);
    }
...
function updateGridProperty(target, attribute, value, element, isGridProperty) {
    if (isGridProperty) {
        ...
        const newHTML = generateGridHTML(target.rows, target.cols, target.hasHeader);ç
...
function updateElementSize(item, prop, value, elementsArray) {
    ...
    if (svg && svgRect) {
        ...
        if (svgArrow) {
            svgArrow.setAttribute('points', `${value - 20},15 ${value - 10},25 ${value - 20},35`);
        }
    }
...
export function updateProperties(id, elementsArray) {
    ...
    if (item) {
        ...
        if (item.type === 'grid') {
            const elementRows = document.getElementById('customProperty-NumberofRows');
            const elementCols = document.getElementById('customProperty-NumberofColumns');
            const elementHasHeader = document.getElementById('customProperty-HasHeader');

            if (elementRows) {
                elementRows.value = item.rows;
                elementRows.onchange = () => updateGridProperty(item, 'rows', parseInt(elementRows.value), item.element, true);
            }
            if (elementCols) {
                elementCols.value = item.cols;
                elementCols.onchange = () => updateGridProperty(item, 'cols', parseInt(elementCols.value), item.element, true);
            }
            if (elementHasHeader) {
                elementHasHeader.checked = item.hasHeader;
                elementHasHeader.onchange = () => updateGridProperty(item, 'hasHeader', elementHasHeader.checked, item.element, true);
            }
        }
    }
}
